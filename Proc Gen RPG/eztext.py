"""eztext.py
made by pywiz32
modified by rtgher 10.12.2014:
    added ' ' to options.
    added get() function, acts just as 'input()'
"""
...
class Input:
        ...
        for event in events:
            ...
            if event.type == KEYDOWN:
                ...
                if not self.shifted:
                    ...
                    elif event.key == K_SPACE and ' ' in self.restricted: self.value += ' '
                elif self.shifted:
                    if event.key == K_a and 'A' in self.restricted: self.value += 'A'
                    elif event.key == K_b and 'B' in self.restricted: self.value += 'B'
                    elif event.key == K_c and 'C' in self.restricted: self.value += 'C'
    ...
    #Modifications
    def get(self) :
        """This function returns a copy of the self.values.
    It also deletes the current values after making the copy.
    """
        if '\n' in self.value :
            text=self.value[:-1]
            self.reset()
    ...
    def reset(self) :
        """This function deletes all values of self.value
    """
        self.value=''
